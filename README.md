...
- 每个任务在 Token 后显示其对 5 小时/周账户窗口的估算占比。该数值由账户已用比例与本机同期 Token 动态推算，只表示近似参考，不等同于官方计费或精确配额。
...
- 只请求 `account/rateLimits/read`，展示权威可用数量、最近过期时间和脱敏详情，并只保留额度窗口的时长、已用百分比与重置时间用于估算。
