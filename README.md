...
<p align="center">
  <img src="docs/assets/screenshots/dashboard.png" width="430" alt="Show Codex IQ 菜单栏与仪表盘，展示紧凑排名摘要及智商、费用、耗时、综合四类榜单">
</p>

<p align="center"><sub>菜单栏与仪表盘：紧凑查看排名摘要、数据状态及四类榜单。</sub></p>
...
```bash
shasum -a 256 -c Show-Codex-IQ-0.1.0-beta.6.1-universal.dmg.sha256
```
...
bash scripts/verify_dmg.sh dist/Show-Codex-IQ-0.1.0-beta.6.1-universal.dmg
```
...
完整版本、Apple 营销版本和构建号统一维护在 `Sources/ShowCodexIQ/Config/Version.xcconfig`。每次发布必须同步更新 [README.md](README.md) 与 [CHANGELOG.md](CHANGELOG.md)，创建 `v<版本号>` 标签，并发布对应 GitHub Release。README 中的功能亮点、截图、安装/构建说明和带版本号的示例必须反映当前发布版本；README、CHANGELOG 与 Release 的用户可见变更应保持一致。具体流程见 [docs/releasing.md](docs/releasing.md)。
